...
module YahooFinance
  ...
  class Client
    ...
    def read_quotes(symb_str, cols)
      ...
      conn = open("http://download.finance.yahoo.com/d/quotes.csv?s=#{CGI::escape(symb_str)}&f=#{columns}")
      CSV.parse(conn.read, headers: cols)
    end

    def read_historical(symbol, options)
      params = {
        s: CGI::escape(symbol), g: HISTORICAL_MODES[options[:period]],
        ignore: '.csv' }
